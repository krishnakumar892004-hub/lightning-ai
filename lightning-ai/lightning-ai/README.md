...
- **📎 Upload Image** — upload a photo and ask questions about it, e.g. a math problem, question paper, or diagram in a picture (model: `qwen/qwen3.6-27b`, vision-capable)
- **Neat formatting** — AI replies now render as proper markdown (headings, bold, bullet lists, code blocks, tables) instead of raw text, similar to Claude/ChatGPT/Gemini
- **Longer answers** — response length increased (max_tokens: 8000) so long, detailed explanations aren't cut off
- **Download button** — on every generated/uploaded image

## Honest note on "matching Claude/ChatGPT/Gemini/Grok"
Those are large proprietary models run by well-funded companies — a free hosted app can't fully match their raw quality. What this setup does give you: Groq's `compound` model (open-weight, with web search + code execution built in) tuned via the system prompt to answer accurately, show step-by-step reasoning, and format cleanly. It's a strong free alternative, not a 1:1 replacement.
